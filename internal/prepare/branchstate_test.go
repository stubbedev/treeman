...
func TestCaptureSkippedOnCleanBounce(t *testing.T) {
	...
	if f.eventCount("branch:capture:skip") != 1 {
		t.Fatalf("expected one capture_skip event, got %d", f.eventCount("branch:capture:skip"))
	}
...
func TestCaptureNeverSkippedWithoutWatermark(t *testing.T) {
	...
	if f.eventCount("branch:capture:skip") != 0 {
		t.Fatalf("no-watermark engine must never emit capture_skip, got %d", f.eventCount("branch:capture:skip"))
	}
}
