...
func TestFanOutClonesHonoursMaxConnsProbe(t *testing.T) {
	...
	restore := func(ctx context.Context, template, target string) error {
		...
		// Make goroutines linger so the limiter is actually exercised
		// and the peak isn't just the first instant of the wave.
		for j := 0; j < 200_000; j++ {
		}
		inflight.Add(-1)
		return nil
	...
	if peak.Load() > int32(want) {
		t.Errorf("peak %d exceeded auto-tuned cap %d", peak.Load(), want)
	}
	if peak.Load() < 2 {
		t.Errorf("expected concurrency, got peak %d", peak.Load())
	}
}
