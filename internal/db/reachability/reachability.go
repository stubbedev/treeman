...
func ProbeCtx(ctx context.Context, engine, host string, port uint16) error {
	...
	if err != nil {
		...
		return fmt.Errorf("%s unreachable at %s:%d (tcp connect refused: %w — if the service is in docker, set `container:`/`compose_service:` so treeman can find its bridge IP or published port)", engine, host, port, err)
	}
	_ = conn.Close()
	return nil
}
