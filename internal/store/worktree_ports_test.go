...
func TestSlugSnapshotRoundTrip(t *testing.T) {
	ctx := context.Background()
	st, repoID, _ := openTestStoreWithWt(t)
	rec := SnapshotRecord{
		Fingerprint:  "wt:feature_x|mysql|kontainer_feature_x",
		Engine:       "mysql",
		SourceDB:     "kontainer_feature_x",
		TemplateName: "treeman_wt_feature_x_kontainer_feature_x",
		SlugOrigin:   SlugSnapshotKey("feature_x", "mysql", "kontainer_feature_x"),
		RepoID:       repoID,
	}
	if err := st.RecordSnapshot(ctx, rec); err != nil {
		t.Fatal(err)
	}
	// Per-slug snapshots must not pollute the fingerprint lookup.
	if got, _ := st.LookupSnapshot(ctx, rec.Fingerprint); got != nil {
		t.Errorf("LookupSnapshot should not return slug-origin rows; got %#v", got)
	}
	// They must be reachable via LookupSlugSnapshot.
	got, err := st.LookupSlugSnapshot(ctx, "feature_x", "mysql", "kontainer_feature_x")
	if err != nil {
		t.Fatal(err)
	}
	if got == nil || got.TemplateName != rec.TemplateName {
		t.Errorf("LookupSlugSnapshot returned %#v", got)
	}
	// And deletable through the dedicated helper.
	deleted, err := st.DeleteSlugSnapshot(ctx, "feature_x", "mysql", "kontainer_feature_x")
	if err != nil {
		t.Fatal(err)
	}
	if !deleted {
		t.Errorf("expected DeleteSlugSnapshot to remove a row")
	}
	if got, _ := st.LookupSlugSnapshot(ctx, "feature_x", "mysql", "kontainer_feature_x"); got != nil {
		t.Errorf("post-delete lookup should return nil, got %#v", got)
	}
}
