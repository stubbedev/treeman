...
const nodeContainer = "treeman-e2e-fw-knex-node"

func TestKnexEndToEnd(t *testing.T) {
	harness.SkipIfNoDocker(t)
	t.Cleanup(harness.ComposeUp(t, harness.MustAbs(".")))
	harness.WaitForReady(t, "postgres:15502", 60*time.Second, func() error {
		c, err := net.DialTimeout("tcp", "127.0.0.1:15502", 1*time.Second)
		if err != nil {
			return err
		}
		_ = c.Close()
	...
	wt := filepath.Join("/tmp", fmt.Sprintf("treeman-knex-%d", time.Now().UnixNano()))
	...
	dsn := fmt.Sprintf("postgres://postgres:pgpw@127.0.0.1:15502/%s?sslmode=disable", o.SourceDB)
